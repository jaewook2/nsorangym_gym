...
class EnergySavingEnv(NsOranEnv):
    ...
    def _get_obs(self):
        ...
        # 추가 JLees
        if ue_kpms is None:
            print(f"[ERROR] No KPMs found for timestamp {self.last_timestamp}")
                # 대체 관측값 생성 or 이전 step 복사 or terminated=True 처리
            self.terminated = True
            return [[0]*len(self.columns_state)]  # 예시: 0으로 채워진 관측값 반환
        #### 
