...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_folder", type=str, default="output/nes",
    ...
    for step in range(2, num_steps):
        ...
        n_action = len(env.action_list)
        model_action_index = np.random.randint(0, n_action)
        dec_action = env.action_list[model_action_index]
        model_action = [int(i) for i in list(f'{dec_action:07b}')]
        print(f'Step {step} ', end='', flush=True)
        obs, reward, terminated, truncated, info = env.step(model_action)
        try:
            obs, reward, terminated, truncated, info = env.step(model_action)
        except TypeError as e:
            print("❗️ Step failed due to missing KPMs:", e)
            break
